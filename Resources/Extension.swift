...
extension CAGradientLayer {
    static func createLinearGradient(hexColors: [UIColor?], view: UIView) -> CAGradientLayer {
        let gradientLayer = CAGradientLayer()
        gradientLayer.colors = hexColors.compactMap { $0?.cgColor }
...
extension UIView {
    func applyLinearGradient(hexColors: [UIColor?]) {
        let gradientLayer = CAGradientLayer.createLinearGradient(hexColors: hexColors, view: self)
        self.layer.insertSublayer(gradientLayer, at: 0)
    }
    func roundRadius(radiusValue: CGFloat, borderWidth: CGFloat,borderColor: UIColor = .black) {
           self.layer.cornerRadius = radiusValue
           self.layer.borderWidth = borderWidth
        self.layer.borderColor = borderColor.cgColor
       }
    func addShadow(color: UIColor = .black, opacity: Float = 0.7, offset: CGSize = CGSize(width: 0, height: 4), radius: CGFloat = 9) {
...
//    }
    ...
    func viewProperty(corner:CGFloat,borderWidth:CGFloat,borderColor:UIColor){
        self.layer.cornerRadius = corner
        self.layer.borderWidth = borderWidth
        self.layer.borderColor = borderColor.cgColor
    }
    
    func viewShadowProperty(opacity:Float,radius:CGFloat,shadowColor:UIColor) {
    ...
    @objc private func doneKybrd() {
        self.endEditing(true)
    }
    func dismissKeyBoard() {
        self.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(doneKybrd)))
    }
    func animateView(_ up:Bool,_ mVal:CGFloat) {
        let mvDrtn:TimeInterval = 0.3;
        let mvmnt:CGFloat = up ? -mVal : mVal;
        UIView.animate(withDuration: mvDrtn) {
            self.frame = self.frame.offsetBy(dx: 0, dy: mvmnt);
        }
//        UIView.beginAnimations("animateView", context: nil);
//        UIView.setAnimationBeginsFromCurrentState(true);
//        UIView.setAnimationDuration(mvDrtn);
//        UIView.commitAnimations()
    ...
    //TODO:- Left to Right Transition Animation
    ...
    //TODO: Right to Left Transition Animation
    func rightToLefttAnimation(duration: TimeInterval = 0.5, completionDelegate: CAAnimationDelegate? = nil) {
        // Create a CATransition object
    ...
    //TODO:- center Transition Animation
    func fromCenteerAnimation(duration: TimeInterval = 0.5, completionDelegate: CAAnimationDelegate? = nil) {
        // Create a CATransition object
        let fromCenterTransition = CATransition()
        
        // Set its callback delegate to the completionDelegate that was provided
        if let delegate: CAAnimationDelegate = completionDelegate {
            fromCenterTransition.delegate = delegate
        }
        
        fromCenterTransition.type = CATransitionType.reveal
        fromCenterTransition.subtype = CATransitionSubtype.fromRight
        fromCenterTransition.duration = duration
        fromCenterTransition.timingFunction = CAMediaTimingFunction(name: CAMediaTimingFunctionName.easeInEaseOut)
        fromCenterTransition.fillMode = CAMediaTimingFillMode.removed
        
        // Add the animation to the View's layer
        self.layer.add(fromCenterTransition, forKey: "fromCenterTransition")
        
    }
    
    //TODO:- Top to Bottom Transition Animation
    func fromTopAnimation(duration: TimeInterval = 0.5, completionDelegate: CAAnimationDelegate? = nil) {
        // Create a CATransition object
        let fromTopTransition = CATransition()
        
        // Set its callback delegate to the completionDelegate that was provided
        if let delegate: CAAnimationDelegate = completionDelegate {
            fromTopTransition.delegate = delegate
        }
        
        fromTopTransition.type = CATransitionType.reveal
        fromTopTransition.subtype = CATransitionSubtype.fromTop
        fromTopTransition.duration = duration
        fromTopTransition.timingFunction = CAMediaTimingFunction(name: CAMediaTimingFunctionName.easeInEaseOut)
        fromTopTransition.fillMode = CAMediaTimingFillMode.removed
        
        // Add the animation to the View's layer
        self.layer.add(fromTopTransition, forKey: "fromTopTransition")
    }
...
extension String {
    ...
    func alertMsg(_ msg:String) -> UIAlertController {
        let alert:UIAlertController = UIAlertController(title: self, message: msg, preferredStyle: .alert);
        //        let alrtArabic = SavePref.savePref.get_LngePref().Name.lowercased().contains("arabic")
        alert.addAction(UIAlertAction(title: "OK", style: .default, handler: nil));
        return alert
    }
    func alertMsg() -> UIAlertController {
        let alert:UIAlertController = UIAlertController(title: self, message: nil, preferredStyle: .alert);
        //        let alrtArabic = SavePref.savePref.get_LngePref().Name.lowercased().contains("arabic")
        alert.addAction(UIAlertAction(title: "OK", style: .default, handler: nil));
        return alert
    }
    ...
    func alertMsg(_ msg:String,_ btnTitle:String,_ isCancel:Bool? = false,_ actnBlock:@escaping (UIAlertAction) ->()) -> UIAlertController {
        let alert:UIAlertController = UIAlertController(title: self, message: msg, preferredStyle: .alert)
        alert.addAction(UIAlertAction(title: btnTitle, style: .default, handler: actnBlock))
        if isCancel ?? false {
    ...
    func alertMsg(msg:String, btnTitle:String, btnAction:@escaping(UIAlertAction)->Void, cancelAction:@escaping(UIAlertAction)->Void) -> UIAlertController {
        let alert:UIAlertController = UIAlertController(title: self, message: msg, preferredStyle: .alert)
        alert.addAction(UIAlertAction(title: btnTitle, style: .default, handler: btnAction))
    ...
    //MARK:- Check is valid phone number or not.
    ...
    var jsonStr2Dic:[String:Any] {
        var t = [String:Any]();
        if let d = self.data(using: .utf8) {
            do {
                if let n = try JSONSerialization.jsonObject(with: d, options: .mutableContainers) as? [String:Any] {
                    print("json converted ==> \(n)")
                    t = n
                }
    ...
    func removeSpecialCharactersFromString()-> String {
        let RISTRICTED_CHARACTERS = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789"
        let set = CharacterSet(charactersIn: RISTRICTED_CHARACTERS)
        let inverted = set.inverted
        let stringValue = self.removeWhitespace
        return stringValue.components(separatedBy: inverted).joined(separator: "")
    }
    
    //MARK:- Grouping/ Format the string.
    ...
    //MARK:- String contains only Digits.
    func getDigitsString() -> String {
    ...
    func toDate(format: String) -> Date {
        ...
        if let strDate = dateFmt.date(from: self) {
            return strDate
        }
        return Date()
    }
    ...
    func convertDate(from:String, to:String) -> String {
        let frmDateFormat = DateFormatter()
        frmDateFormat.dateFormat = from
        let toDateFormat = DateFormatter()
        toDateFormat.dateFormat = to
        if let strDate = frmDateFormat.date(from: self) {
            return toDateFormat.string(from: strDate)
        }
        return self
    }
    
    //MARK:- Convert Date to perticular Format as String
    ...
    var toArray:[String] {
        var arrStr = [String]()
        if self.contains(",") {
            arrStr = self.components(separatedBy: ",")
        } else {
            arrStr = [self]
        }
        return arrStr
    ...
    func height(withConstrainedWidth width: CGFloat, fsize: CGFloat) -> CGFloat {
        let constraintRect = CGSize(width: width, height: .greatestFiniteMagnitude); let fontAttributes:[NSAttributedString.Key: Any] = [.font: UIFont.systemFont(ofSize: fsize)]
        let boundingBox = self.boundingRect(with: constraintRect, options: .usesLineFragmentOrigin, attributes: fontAttributes, context: nil)
        return ceil(boundingBox.height)
    }
    func width(withConstraintedHeight height: CGFloat, fsize: CGFloat) -> CGFloat {
        let constraintRect = CGSize(width: .greatestFiniteMagnitude, height: height); let fontAttributes:[NSAttributedString.Key: Any] = [.font: UIFont.systemFont(ofSize: fsize)]
        let boundingBox = self.boundingRect(with: constraintRect, options: .usesLineFragmentOrigin, attributes: fontAttributes, context: nil)
        return ceil(boundingBox.width)
    }
    func size(_ fsize:CGFloat) -> CGSize {
        let fontAttributes:[NSAttributedString.Key: Any] = [.font: UIFont.systemFont(ofSize: fsize)]
